 ...
 *  
 *  Authors: Harsh Shrivastava && Patrick Lavin
 */
...
void scatter(const int n, double* scatter_values, int &n_local, double* &local_values, int source_rank, const MPI_Comm comm){
  ...
  if(rank==source_rank){
	...
	//begin
	if(valid == message_size){
	  //NOP
	}else if(valid > 0){
	  local_values = (double *)realloc(local_values, message_size*sizeof(double));
	  valid = message_size;
	}else{
    ...
    if(valid == n_local){
      ...
      local_values = (double *)realloc(local_values, n_local*sizeof(double));
      valid = n_local;
    }else{
...
double broadcast(double value, int source_rank, const MPI_Comm comm){
    ...
    for(int i=0; i<d; i++){// going from LSB to MSB
      ...
      if(rank_c < p){
	//Lower rank is the sender && only send if you actually have the value
	if(rank < rank_c && rank < (1<<i)){
	  MPI_Send(&value, 1, MPI_DOUBLE, posmod(rank_c+source_rank, p), 1, comm);
	}
        //Only recv if our complement has the value
	else if(rank_c < (1<<i)){
